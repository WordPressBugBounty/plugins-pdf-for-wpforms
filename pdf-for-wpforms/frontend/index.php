...
class Yeepdf_Create_PDF
{
	function __construct()
	{
		...
		add_filter('pdf_before_render_datas', array($this, 'shortcode_multi_images'));
	}
	function shortcode_multi_images($data_send_settings_download)
	{
		...
		return $data_send_settings_download;
	}
						...
						case "table":
							if ($k == "html_hide_table") {
								$clean = preg_replace([
									...
									'/<sider-quick-compose-btn\b[^>]*>.*?<\/sider-quick-compose-btn>/is',
									'/<div[^>]*class="[^"]*rc-handle[^"]*"[^>]*>.*?<\/div>/is'
								], '', $v);
								$html_el->find($key, 0)->__set("innertext", $clean);
							}
							break;;
						case "form_select":
							if ($k == "value_select") {
