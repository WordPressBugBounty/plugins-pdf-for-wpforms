...
class Yeepdf_Settings_Builder_PDF_Backend
{
    function __construct()
    {
        ...
        add_action("yeepdf_header_settings", array($this, "add_header_settings"), 10);
        add_action("yeepdf_footer_settings", array($this, "add_header_settings"), 10);
        add_action("yeepdf_watermark_text_settings", array($this, "add_header_settings"), 10);
        add_action("yeepdf_watermark_img_settings", array($this, "add_header_settings"), 10);
    }
    function add_header_settings($pdfs)
    {
        echo wp_kses_post($this->get_text_pro());
    }
    function get_text_pro()
    {
        $text = '<div class="pro_disable pro_disable_fff">Upgrade to pro version</div>';
        return $text;
    }
...
?>
    ...
    function style()
    {
        ...
        if ($add_libs) {
            ...
            wp_register_script('table_resize', YEEPDF_CREATOR_BUILDER_URL . "backend/libs/table-resize/jquery.resizableColumns.js", array("jquery"), $ver, true);
            wp_enqueue_script('yeepdf_script', YEEPDF_CREATOR_BUILDER_URL . "backend/src/script.js", array("jquery", "yeepdf_main", "jquery-ui-core", "jquery-ui-dialog", "jquery-ui-sortable", "jquery-ui-draggable", "jquery-ui-droppable", "wp-color-picker", "wp-tinymce", "yeepdf_editor", "yeepdf_builder", "jquery-effects-core", "jquery-effects-scale", "yeepdf_pdf_code_toggle", "thickbox", "table_resize"), $ver);
            $list_fonts = Yeepdf_Settings_Main::get_list_fonts();
            $google_fonts = get_option("pdf_custom_fonts", array());
            $font_formats = "";
